...
let CourseClassAssignmentAuthorization = class CourseClassAssignmentAuthorization extends BaseAuthorization_1.default {
    authorizeCreateAssignment(user, course, enrollment) {
    ...
    authorizeReadAssignment(user, course, enrollment) {
        const { id: userId, role: userRole } = user;
        const { authorId } = course;
        const isAuthor = userId === authorId;
        const { isAdmin, isInstructor, isStudent } = (0, getRoleStatus_1.default)(userRole);
    ...
    authorizeUpdateAssignment(user, course, enrollment) {
        this.authorizeCreateAssignment(user, course, enrollment);
    }
    authorizeDeleteAssignment(user, course, enrollment) {
        this.authorizeCreateAssignment(user, course, enrollment);
    }
};
